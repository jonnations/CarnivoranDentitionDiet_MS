...
plot(p1, plot = FALSE)[[1]] + 
  ...
  scale_size_manual(values = c(1, 1.4, 2, 10))
}
```
...
conditions <- data.frame(log_cuberoot_mass = c(-1.5, 0, 1.5))
mass.labs <- c("Low Mass", "Mean Mass", "High Mass")
conditional_effects(readRDS(here("Code/Models/mod_outputs/bird_md_RLGA.Rds")), 
                          categorical  = TRUE, 
                          prob = 0.75, 
                          conditions = conditions, 
                          effects= "RLGA")[[1]] %>% 
...
ggplot(aes(x = "RLGA", y = Probability, group = cats__)) +
...
ggplot(aes(x={{effect2}}, y = Probability, group = cats__)) +
  ...
  theme( panel.grid.major = element_blank(), 
        ...
        # strip.text.x = element_text(size = 8),
         ...
         axis.title.x = element_text(size = 6, margin=margin(0,0,-5,0)),
...
plot3col
ggsave(here("Plots", "Figure_3.pdf"), width = 7.5, height = 3.375)
ggsave(here("Plots", "Figure_3.png"), width = 7.5, height = 3.375)
```

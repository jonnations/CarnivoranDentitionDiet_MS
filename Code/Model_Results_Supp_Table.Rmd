...
read_csv(here("Data", "Supp_model_results_table.csv"))  %>% pivot_longer(!c(Metric,Food), names_to = "Parameter", values_to = "Test") %>% pivot_wider(names_from = Food, values_from = Test) %>% kbl(longtable = T, escape = F, align = 'c', format = 'latex') %>% kable_classic(full_width = F, html_font = "Cambria", font_size = 5) %>% writeLines(here("Plots", "Supp_model_results_table.tex"))

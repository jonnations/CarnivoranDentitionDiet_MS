...
mean_prep <- function(x){
  x %>% 
    ...
    slice(rep(1:n(), each = n_ext)) %>% as.matrix()
}
...
mean_prep2 <- function(x){
  x %>% 
    summarise(across(1:13, mean)) %>% 
    slice(rep(1:n(), each = n_crpt)) %>% as.matrix()
}
...
get_neighbors2 <- function(taxon, taxa) {
  distances <- numeric()
  taxa[taxa != taxon]
  for(i in 1:nrow(taxa)) distances[i] <- dist(rbind(taxon, taxa[i,]), method = "euclidean")
  names(distances) <- rownames(taxa)
...
S_new2 <- purrr::pmap(list(ed2_wide, means_list, es_list), pc_scores)
```
...
#### Write  
```{r}
nn %>% write_csv(here("Data", "NN_probabilities.csv"))

# Table with just the closest 3 extant species to each fossil/cryptic taxon

nn %>% group_by(Species) %>% slice(1:3) %>% ungroup() %>% write_csv(here("Data", "NN_probabilities_top3.csv"))
```

#### Preds and weight

```{r}
ms <- dat %>% select(Species, log_cuberoot_mass) %>% 
  mutate(nearest_1 = Species)

d1 <- nn %>% right_join(ms, by = 'Species') %>% select(!nearest_1.y) %>% rename(nearest_1 = nearest_1.x)
d2 <- nn %>% rename(mass2 = log_cuberoot_mass) %>% right_join(ms, by = 'nearest_1')
```


## NNs Extant
```{r}
nns2 <- function(S_cryptic2, S_new2){
  cryptic_res <- matrix(data=NA, nrow=nrow(S_cryptic2), ncol=4)
  for(i in 1:nrow(S_cryptic2)) cryptic_res[i,] <- names(get_neighbors(S_cryptic2[i,2:5], S_new2[,2:5]))[c(2:5)]
  colnames(cryptic_res) <- paste("nearest", 2:5, sep="_")
  cryptic_res <- as_tibble(cryptic_res) %>% mutate( Species = rownames(S_cryptic2))
  return(cryptic_res)
}


nntest2 <- pmap(list(S_new2, S_new2), nns2)

#Flatten array into a dataframe, then calculate the percentages of the closest nearest neighbors. 
nn2 <- bind_rows(nntest2) %>% 
  group_by(Species) %>% 
  select(Species, nearest_2) %>% 
  count(nearest_2) %>% 
  mutate(percent = n/4000) %>% 
  arrange(Species, desc(percent)) %>% 
  filter(percent > 0.05) # %>% 
...
```{r}
nn2 %>% filter(Species == "Ursus_maritimus")
```

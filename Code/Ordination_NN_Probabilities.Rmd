...
Split this on .draw
Then for each df in the array, do the nn estimates. 
...
# get the means of each column (for "means") then rep it 89 times for the nrow() of extant data
...
Now do the same for the cryptics
```{r}
...
nns <- function(S_cryptic2, S_new2){
  ...
  #rownames(cryptic_res) <- rownames(S_cryptic2)
  #colnames(cryptic_res) <- paste("nearest", 1:5, sep="_")
  return(cryptic_res)
}

#make the array (or a list of tibbles)

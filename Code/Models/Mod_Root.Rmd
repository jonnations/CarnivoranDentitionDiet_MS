...
---
# root
 Similar to root and some others, there are almost all 1's for root, so the model just predicts that they are all 1's. 
Setup
```{r}
...
#### Plot
```{r}
dat1 %>% ggplot(aes(x =root)) + geom_bar()
```
...
m2 <-  brm(root ~ 1, data = dat1,family = cumulative(), prior = p2,
           refresh = 0, chains = 4,  cores = 4, sample_prior = 'only')
m3 <-  brm(root ~ 1, 
...
#### Write Weights
```{r}
mw_root <- model_weights( mn_DNE, md_ALL, md_OPCR, mn_RFI, mn_RLGA, weights = "stacking") %>% 
  round(digits = 3) %>% 
  as.data.frame() %>%
  rownames_to_column("metric") %>% 
  add_column(diet = "root") %>% 
...
```{r}
mn_RLGA %>% write_rds("new_mod_outputs/root_mn_RLGA.Rds")
```

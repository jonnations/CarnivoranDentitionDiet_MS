...
---
# seed
 Similar to seed and some others, there are almost all 1's for seed, so the model just predicts that they are all 1's. 
Setup
```{r}
...
#### Write Weights
```{r}
mw_seed <- model_weights( mn_DNE, mn_ALL, md_OPCR, md_RFI, mn_RLGA, weights = "stacking") %>% 
  round(digits = 3) %>% 
  as.data.frame() %>%
  rownames_to_column("metric") %>% 
  add_column(diet = "seed") %>% 
...
```{r}
mn_RLGA %>% write_rds("new_mod_outputs/seed_mn_RLGA.Rds")
mn_DNE %>% write_rds("new_mod_outputs/seed_mn_DNE.Rds")
```

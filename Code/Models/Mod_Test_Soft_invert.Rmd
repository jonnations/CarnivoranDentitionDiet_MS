...
---
# soft_invert
 Similar to soft_invert and some others, there are almost all 1's for soft_invert, so the model just predicts that they are all 1's. 
Setup
```{r}
...
#### Plot
```{r}
dat1 %>% ggplot(aes(x =soft_invert)) + geom_bar()
```
...
p2 <- c(set_prior("normal(1,1)", class = "Intercept"))
...
p7 <- c(set_prior("student_t(2,1,2)", class = "Intercept"))

p8 <- c(set_prior("student_t(3,1,3)", class = "Intercept"))
...
```{r, include = FALSE}
mn_RLGA <-  brm(soft_invert ~  RLGA * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
...
ms_RLGA <-  brm(soft_invert ~  RLGA * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior = c(set_prior("student_t(2,1,2)", class = "Intercept"),
        set_prior("normal(0,1)", class = "sd"),
        set_prior("normal(0,1)", class = "b")),
           refresh = 0, chains = 4,  cores = 4, control = list(adapt_delta = 0.98)) %>% add_criterion(.,  criterion = c("loo", 'waic'))

md_RLGA <-  brm(soft_invert ~  RLGA * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
...
pred2 <- as.data.frame(pred) %>% mutate(actual = as.factor(dat1$soft_invert), pred   = colnames(pred)[max.col(pred,ties.method="first")], pred   = str_remove(pred,"P\\(Y = "), pred   = str_remove(pred,"\\)"), actual = factor(actual, ordered = FALSE), pred   = factor(pred), Species = pull(dat1, Species))
...
Using 3 priors.
```{r, include=FALSE}
mn_DNE <-  brm(soft_invert ~  m1_dne * log_cuberoot_mass + m2_dne * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior = c(set_prior("normal(1,1)", class = "Intercept"),
        set_prior("normal(0,1)", class = "sd"),
        set_prior("normal(0,1)", class = "b")),
...
ms_DNE <-  brm(soft_invert ~  m1_dne * log_cuberoot_mass + m2_dne * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior = c(set_prior("student_t(2,1,2)", class = "Intercept"),
        set_prior("normal(0,1)", class = "sd"),
        set_prior("normal(0,1)", class = "b")),
...
md_DNE <-  brm(soft_invert ~  m1_dne * log_cuberoot_mass + m2_dne * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior =  c(set_prior("induced_dirichlet(rep_vector(1, nthres+1),0", class = "Intercept"),
                     set_prior("normal(0,1)", class = "sd"),
                     set_prior("normal(0,1)", class = "b")),
...
# RFI
```{r, include = FALSE}
mn_RFI <-  brm(soft_invert ~  RFI_m1 * log_cuberoot_mass + RFI_m2 * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior = c(set_prior("normal(1,1)", class = "Intercept"),
        set_prior("normal(0,1)", class = "sd"),
        set_prior("normal(0,1)", class = "b")),
...
ms_RFI <-  brm(soft_invert ~  RFI_m1 * log_cuberoot_mass + RFI_m2 * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(),  
           prior = c(set_prior("student_t(2,1,2)", class = "Intercept"),
        set_prior("normal(0,1)", class = "sd"),
        set_prior("normal(0,1)", class = "b")),
...
md_RFI <-  brm(soft_invert ~  RFI_m1 * log_cuberoot_mass + RFI_m2 * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(),  
           prior =  c(set_prior("induced_dirichlet(rep_vector(1, nthres+1),0", class = "Intercept"),
                     set_prior("normal(0,1)", class = "sd"),
                     set_prior("normal(0,1)", class = "b")),
...
Using 3 priors.
```{r, include = FALSE}
mn_OPCR <-  brm(soft_invert ~  m1_opcr * log_cuberoot_mass + m2_opcr * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior = c(set_prior("normal(1,1)", class = "Intercept"),
        set_prior("normal(0,1)", class = "sd"),
        set_prior("normal(0,1)", class = "b")),
...
ms_OPCR <-  brm(soft_invert ~  m1_opcr * log_cuberoot_mass + m2_opcr * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior = c(set_prior("student_t(2,1,2)", class = "Intercept"),
        set_prior("normal(0,1)", class = "sd"),
        set_prior("normal(0,1)", class = "b")),
...
md_OPCR <-  brm(soft_invert ~  m1_opcr * log_cuberoot_mass + m2_opcr * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior =  c(set_prior("induced_dirichlet(rep_vector(1, nthres+1),0", class = "Intercept"),
                     set_prior("normal(0,1)", class = "sd"),
                     set_prior("normal(0,1)", class = "b")),
...
```{r, include = FALSE}
mn_ALL <-  brm(soft_invert ~  log_cuberoot_mass*m1_opcr + 
...
ms_ALL <-  brm(soft_invert ~  log_cuberoot_mass*m1_opcr + 
                      log_cuberoot_mass*m2_opcr +
                      log_cuberoot_mass*m1_dne  + 
                      log_cuberoot_mass*m2_dne  + 
                      log_cuberoot_mass*RFI_m1  + 
...
md_ALL <-  brm(soft_invert~  log_cuberoot_mass*m1_opcr + 
                      log_cuberoot_mass*m2_opcr +
                      log_cuberoot_mass*m1_dne  + 
                      log_cuberoot_mass*m2_dne  + 
                      log_cuberoot_mass*RFI_m1  + 
...
mw_soft_invert <- model_weights(mn_DNE, ms_DNE, md_DNE, mn_ALL, ms_ALL, md_ALL, mn_OPCR, ms_OPCR, md_OPCR, mn_RFI, ms_RFI, md_RFI, mn_RLGA,ms_RLGA, md_RLGA, weights = "stacking") %>% 
  round(digits = 3) %>% 
  as.data.frame() %>%
  rownames_to_column("metric") %>% 
  add_column(diet = "soft_invert") %>% 
  rename("value" = ".") %>% 
  arrange(desc(value))
...
#### Write Weights
```{r}
mw_soft_invert <- model_weights( mn_DNE, mn_ALL, md_OPCR, ms_RFI, md_RLGA, weights = "stacking") %>% 
  round(digits = 3) %>% 
  as.data.frame() %>%
  rownames_to_column("metric") %>% 
  add_column(diet = "soft_invert") %>% 
...
```{r}
mn_DNE %>% write_rds("new_mod_outputs/soft_invert_mn_DNE.Rds")
md_OPCR %>% write_rds("new_mod_outputs/soft_invert_md_OPCR.Rds")
```

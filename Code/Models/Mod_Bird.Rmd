...
```{r}
set.seed(826)
mw_bird <- model_weights( md_DNE, mn_ALL,  ms_OPCR, ms_RFI,  md_RLGA, weights = "stacking") %>% 
  round(digits = 3) %>% 
  as.data.frame() %>%
  rownames_to_column("metric") %>% 
  add_column(diet = "bird") %>% 
...
```{r}
write_csv(mw_bird, file = here("Data", "Test_weights_all.csv"))
```
...
```{r}
md_RLGA %>% write_rds(file = here("Code/Models/mod_outputs", "bird_md_RLGA.Rds"))
mn_ALL %>%  write_rds(file = here("Code/Models/mod_outputs", "bird_mn_ALL.Rds"))
ms_RFI %>%  write_rds(file = here("Code/Models/mod_outputs", "bird_ms_RFI.Rds"))
ms_OPCR %>%  write_rds(file = here("Code/Models/mod_outputs", "bird_ms_OPCR.Rds"))
md_DNE %>%  write_rds(file = here("Code/Models/mod_outputs", "bird_md_DNE.Rds"))
```

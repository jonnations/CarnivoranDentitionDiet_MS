...
---
# large_mammal
 Similar to root and some others, there are almost all 1's for large_mammal, so the model just predicts that they are all 1's. 
Setup
```{r}
...
#### Plot
```{r}
dat1 %>% ggplot(aes(x =large_mammal)) + geom_bar()
```
...
#### Write Weights
```{r}
mw_large_mammal <- model_weights( md_DNE, ms_ALL, ms_OPCR, md_RFI, ms_RLGA, weights = "stacking") %>% 
  round(digits = 3) %>% 
  as.data.frame() %>%
  rownames_to_column("metric") %>% 
  add_column(diet = "large_mammal") %>% 
...
```{r}
ms_RLGA %>% write_rds("new_mod_outputs/large_mammal_ms_RLGA.Rds")
ms_ALL %>%  write_rds("new_mod_outputs/large_mammal_ms_ALL.Rds")
```

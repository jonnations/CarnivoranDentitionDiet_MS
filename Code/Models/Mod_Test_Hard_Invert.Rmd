...
---
# hard_invert
 Similar to root and some others, there are almost all 1's for hard_invert, so the model just predicts that they are all 1's. 
Setup
```{r}
...
#### Write Weights
```{r}
mw_hard_invert <- model_weights( mn_DNE, md_ALL, ms_OPCR, ms_RFI, md_RLGA, weights = "stacking") %>% 
  round(digits = 3) %>% 
  as.data.frame() %>%
  rownames_to_column("metric") %>% 
  add_column(diet = "hard_invert") %>% 
...
```{r}
md_RLGA %>% write_rds("new_mod_outputs/hard_invert_md_RLGA.Rds")
md_ALL %>%  write_rds("new_mod_outputs/hard_invert_md_ALL.Rds")
mn_DNE %>%  write_rds("new_mod_outputs/hard_invert_mn_DNE.Rds")
```

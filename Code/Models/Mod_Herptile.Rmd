...
```{r}
set.seed(826)
mw_herptile <- model_weights( ms_DNE, md_ALL, md_OPCR, md_RFI, md_RLGA, weights = "stacking") %>% 
  round(digits = 3) %>% 
  as.data.frame() %>%
  rownames_to_column("metric") %>% 
  add_column(diet = "herptile") %>% 
...
```{r}
md_RLGA %>% write_rds(file = here("Code/Models/mod_outputs", "herptile_md_RLGA.Rds"))
md_ALL %>%  write_rds(file = here("Code/Models/mod_outputs", "herptile_md_ALL.Rds"))
ms_DNE %>%  write_rds(file = here("Code/Models/mod_outputs", "herptile_ms_DNE.Rds"))
md_RFI %>%  write_rds(file = here("Code/Models/mod_outputs", "herptile_md_RFI.Rds"))
md_OPCR %>%  write_rds(file = here("Code/Models/mod_outputs", "herptile_md_OPCR.Rds"))
```

...
```{r}
set.seed(826)
mw_carrion <- model_weights( mn_RLGA, md_DNE, ms_RFI, md_OPCR,  ms_ALL,  weights = "stacking") %>% 
  round(digits = 3) %>% 
  as.data.frame() %>%
  rownames_to_column("metric") %>% 
  add_column(diet = "carrion") %>% 
...
```{r}
mn_RLGA %>% write_rds(file = here("Code/Models/mod_outputs", "carrion_mn_RLGA.Rds"))
md_OPCR %>% write_rds(file = here("Code/Models/mod_outputs", "carrion_md_OPCR.Rds"))
md_DNE %>% write_rds(file = here("Code/Models/mod_outputs", "carrion_md_DNE.Rds"))
ms_RFI %>% write_rds(file = here("Code/Models/mod_outputs", "carrion_ms_RFI.Rds"))
ms_ALL %>% write_rds(file = here("Code/Models/mod_outputs", "carrion_ms_ALL.Rds"))
```

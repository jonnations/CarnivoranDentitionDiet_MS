...
```{r}
set.seed(826)
mw_fruit <- model_weights( ms_DNE, md_ALL, mn_OPCR, mn_RFI, mn_RLGA, weights = "stacking") %>% 
  round(digits = 3) %>% 
  as.data.frame() %>%
  rownames_to_column("metric") %>% 
  add_column(diet = "fruit") %>% 
...
```{r}
mn_OPCR %>% write_rds(file = here("Code/Models/mod_outputs", "fruit_mn_OPCR.Rds"))
mn_RLGA %>% write_rds(file = here("Code/Models/mod_outputs", "fruit_mn_RLGA.Rds"))
mn_RFI %>%  write_rds(file = here("Code/Models/mod_outputs", "fruit_mn_RFI.Rds"))
ms_DNE %>%  write_rds(file = here("Code/Models/mod_outputs", "fruit_ms_DNE.Rds"))
md_ALL %>%  write_rds(file = here("Code/Models/mod_outputs", "fruit_md_ALL.Rds"))
```

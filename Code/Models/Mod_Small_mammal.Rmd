...
---
# small_mammal
 Similar to small_mammal and some others, there are almost all 1's for small_mammal, so the model just predicts that they are all 1's. 
Setup
```{r}
...
```{r, include = FALSE}
mn_RLGA <-  brm(small_mammal ~  RLGA * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
...
ms_RLGA <-  brm(small_mammal ~  RLGA * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior = c(set_prior("student_t(3,-1,2)", class = "Intercept"),
        set_prior("normal(0,1)", class = "sd"),
        set_prior("normal(0,1)", class = "b")),
           refresh = 0, chains = 4,  cores = 4, control = list(adapt_delta = 0.98)) %>% add_criterion(.,  criterion = c("loo", 'waic'))

md_RLGA <-  brm(small_mammal ~  RLGA * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
...
pred2 <- as.data.frame(pred) %>% mutate(actual = as.factor(dat1$small_mammal), pred   = colnames(pred)[max.col(pred,ties.method="first")], pred   = str_remove(pred,"P\\(Y = "), pred   = str_remove(pred,"\\)"), actual = factor(actual, ordered = FALSE), pred   = factor(pred), Species = pull(dat1, Species))
...
Using 3 priors.
```{r, include=FALSE}
mn_DNE <-  brm(small_mammal ~  m1_dne * log_cuberoot_mass + m2_dne * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior = c(set_prior("normal(-1,1)", class = "Intercept"),
        set_prior("normal(0,1)", class = "sd"),
        set_prior("normal(0,1)", class = "b")),
...
ms_DNE <-  brm(small_mammal ~  m1_dne * log_cuberoot_mass + m2_dne * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior = c(set_prior("student_t(3,-1,2)", class = "Intercept"),
        set_prior("normal(0,1)", class = "sd"),
        set_prior("normal(0,1)", class = "b")),
...
md_DNE <-  brm(small_mammal ~  m1_dne * log_cuberoot_mass + m2_dne * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior =  c(set_prior("induced_dirichlet(rep_vector(1, nthres+1),0", class = "Intercept"),
                     set_prior("normal(0,1)", class = "sd"),
                     set_prior("normal(0,1)", class = "b")),
...
# RFI
```{r, include = FALSE}
mn_RFI <-  brm(small_mammal ~  RFI_m1 * log_cuberoot_mass + RFI_m2 * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior = c(set_prior("normal(-1,1)", class = "Intercept"),
        set_prior("normal(0,1)", class = "sd"),
        set_prior("normal(0,1)", class = "b")),
...
ms_RFI <-  brm(small_mammal ~  RFI_m1 * log_cuberoot_mass + RFI_m2 * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(),  
           prior = c(set_prior("student_t(3,-1,2)", class = "Intercept"),
        set_prior("normal(0,1)", class = "sd"),
        set_prior("normal(0,1)", class = "b")),
...
md_RFI <-  brm(small_mammal ~  RFI_m1 * log_cuberoot_mass + RFI_m2 * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(),  
           prior =  c(set_prior("induced_dirichlet(rep_vector(1, nthres+1),0", class = "Intercept"),
                     set_prior("normal(0,1)", class = "sd"),
                     set_prior("normal(0,1)", class = "b")),
...
Using 3 priors.
```{r, include = FALSE}
mn_OPCR <-  brm(small_mammal ~  m1_opcr * log_cuberoot_mass + m2_opcr * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior = c(set_prior("normal(-1,1)", class = "Intercept"),
        set_prior("normal(0,1)", class = "sd"),
        set_prior("normal(0,1)", class = "b")),
...
ms_OPCR <-  brm(small_mammal ~  m1_opcr * log_cuberoot_mass + m2_opcr * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior = c(set_prior("student_t(3,-1,2)", class = "Intercept"),
        set_prior("normal(0,1)", class = "sd"),
        set_prior("normal(0,1)", class = "b")),
...
md_OPCR <-  brm(small_mammal ~  m1_opcr * log_cuberoot_mass + m2_opcr * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior =  c(set_prior("induced_dirichlet(rep_vector(1, nthres+1),0", class = "Intercept"),
                     set_prior("normal(0,1)", class = "sd"),
                     set_prior("normal(0,1)", class = "b")),
...
```{r, include = FALSE}
mn_ALL <-  brm(small_mammal ~  log_cuberoot_mass*m1_opcr + 
...
ms_ALL <-  brm(small_mammal ~  log_cuberoot_mass*m1_opcr + 
                      log_cuberoot_mass*m2_opcr +
                      log_cuberoot_mass*m1_dne  + 
                      log_cuberoot_mass*m2_dne  + 
                      log_cuberoot_mass*RFI_m1  + 
...
md_ALL <-  brm(small_mammal~  log_cuberoot_mass*m1_opcr + 
                      log_cuberoot_mass*m2_opcr +
                      log_cuberoot_mass*m1_dne  + 
                      log_cuberoot_mass*m2_dne  + 
                      log_cuberoot_mass*RFI_m1  + 
...
mw_small_mammal <- model_weights(mn_DNE, ms_DNE, md_DNE, mn_ALL, ms_ALL, md_ALL, mn_OPCR, ms_OPCR, md_OPCR, mn_RFI, ms_RFI, md_RFI, mn_RLGA,ms_RLGA, md_RLGA, weights = "stacking") %>% 
  round(digits = 3) %>% 
  as.data.frame() %>%
  rownames_to_column("metric") %>% 
  add_column(diet = "small_mammal") %>% 
  rename("value" = ".") %>% 
  arrange(desc(value))
...
#### Write Weights
```{r}
mw_small_mammal <- model_weights( md_DNE, mn_ALL, md_OPCR, mn_RFI, md_RLGA, weights = "stacking") %>% 
  round(digits = 3) %>% 
  as.data.frame() %>%
  rownames_to_column("metric") %>% 
  add_column(diet = "small_mammal") %>% 
...
```{r}
md_RLGA %>% write_rds("new_mod_outputs/small_mammal_md_RLGA.Rds")
md_DNE %>% write_rds("new_mod_outputs/small_mammal_md_DNE.Rds")
```

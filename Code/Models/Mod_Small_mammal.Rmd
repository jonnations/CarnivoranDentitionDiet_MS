...
```{r}
set.seed(826)
mw_small_mammal <- model_weights( md_DNE, mn_ALL, ms_OPCR, md_RFI, ms_RLGA, weights = "stacking") %>% 
  round(digits = 3) %>% 
  as.data.frame() %>%
  rownames_to_column("metric") %>% 
  add_column(diet = "small_mammal") %>% 
...
```{r}
ms_RLGA %>% write_rds(file = here("Code/Models/mod_outputs", "small_mammal_ms_RLGA.Rds"))
mn_ALL %>%  write_rds(file = here("Code/Models/mod_outputs", "small_mammal_mn_ALL.Rds"))
md_DNE %>%  write_rds(file = here("Code/Models/mod_outputs", "small_mammal_md_DNE.Rds"))
md_RFI %>%  write_rds(file = here("Code/Models/mod_outputs", "small_mammal_md_RFI.Rds"))
ms_OPCR %>%  write_rds(file = here("Code/Models/mod_outputs", "small_mammal_ms_OPCR.Rds"))
```

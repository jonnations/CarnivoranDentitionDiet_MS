...
---
# fruit
 Similar to root and some others, there are almost all 1's for fruit, so the model just predicts that they are all 1's. 
Setup
```{r}
...
#### Plot
```{r}
dat1 %>% ggplot(aes(x =fruit)) + geom_bar()
```
...
p5 <- c(set_prior("student_t(3,0,2)", class = "Intercept"))

p6 <- c(set_prior("student_t(3,0,3)", class = "Intercept"))

p7 <- c(set_prior("student_t(3,0,4)", class = "Intercept"))

p8 <- c(set_prior("student_t(8,0,4)", class = "Intercept"))
...
m2 <-  brm(fruit ~ 1, data = dat1,family = cumulative(), prior = p2,
           refresh = 0, chains = 4,  cores = 4, sample_prior = 'only')
m3 <-  brm(fruit ~ 1, 
...
```{r, include = FALSE}
mn_RLGA <-  brm(fruit ~  RLGA * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
...
ms_RLGA <-  brm(fruit ~  RLGA * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
...
md_RLGA <-  brm(fruit ~  RLGA * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
...
pred2 <- as.data.frame(pred) %>% mutate(actual = as.factor(dat1$herptile), pred   = colnames(pred)[max.col(pred,ties.method="first")], pred   = str_remove(pred,"P\\(Y = "), pred   = str_remove(pred,"\\)"), actual = factor(actual, ordered = FALSE), pred   = factor(pred), Species = pull(dat1, Species))
...
Using 3 priors.
```{r, include=FALSE}
mn_DNE <-  brm(fruit ~  m1_dne * log_cuberoot_mass + m2_dne * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior = c(set_prior("normal(0,2)", class = "Intercept"),
        set_prior("normal(0,1)", class = "sd"),
        set_prior("normal(0,1)", class = "b")),
...
ms_DNE <-  brm(fruit ~  m1_dne * log_cuberoot_mass + m2_dne * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior = c(set_prior("student_t(3,0,2)", class = "Intercept"),
        set_prior("normal(0,1)", class = "sd"),
        set_prior("normal(0,1)", class = "b")),
...
md_DNE <-  brm(fruit ~  m1_dne * log_cuberoot_mass + m2_dne * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior =  c(set_prior("induced_dirichlet(rep_vector(1, nthres+1),0", class = "Intercept"),
                     set_prior("normal(0,1)", class = "sd"),
                     set_prior("normal(0,1)", class = "b")),
...
pred2 <- as.data.frame(pred) %>% mutate(actual = as.factor(dat1$fruit), pred   = colnames(pred)[max.col(pred,ties.method="first")], pred   = str_remove(pred,"P\\(Y = "), pred   = str_remove(pred,"\\)"), actual = factor(actual, ordered = FALSE), pred   = factor(pred), Species = pull(dat1, Species))
cm <- caret::confusionMatrix(pred2$pred, pred2$actual, dnn = c("Prediction", "Reference"))
cm$overall[1]
...
# RFI
```{r, include = FALSE}
mn_RFI <-  brm(fruit ~  RFI_m1 * log_cuberoot_mass + RFI_m2 * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior = c(set_prior("normal(0,2)", class = "Intercept"),
        set_prior("normal(0,1)", class = "sd"),
        set_prior("normal(0,1)", class = "b")),
...
ms_RFI <-  brm(fruit ~  RFI_m1 * log_cuberoot_mass + RFI_m2 * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(),  
           prior = c(set_prior("student_t(3,0,2)", class = "Intercept"),
        set_prior("normal(0,1)", class = "sd"),
        set_prior("normal(0,1)", class = "b")),
...
md_RFI <-  brm(fruit ~  RFI_m1 * log_cuberoot_mass + RFI_m2 * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(),  
           prior =  c(set_prior("induced_dirichlet(rep_vector(1, nthres+1),0", class = "Intercept"),
                     set_prior("normal(0,1)", class = "sd"),
                     set_prior("normal(0,1)", class = "b")),
...
Using 3 priors.
```{r, include = FALSE}
mn_OPCR <-  brm(fruit ~  m1_opcr * log_cuberoot_mass + m2_opcr * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior = c(set_prior("normal(0,2)", class = "Intercept"),
        set_prior("normal(0,1)", class = "sd"),
        set_prior("normal(0,1)", class = "b")),
...
ms_OPCR <-  brm(fruit ~  m1_opcr * log_cuberoot_mass + m2_opcr * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior = c(set_prior("student_t(3,0,2)", class = "Intercept"),
        set_prior("normal(0,1)", class = "sd"),
        set_prior("normal(0,1)", class = "b")),
...
md_OPCR <-  brm(fruit ~  m1_opcr * log_cuberoot_mass + m2_opcr * log_cuberoot_mass + (1 | gr(phylo, cov = A)), 
           data = dat1, data2 = list(A = A),family = cumulative(), 
           prior =  c(set_prior("induced_dirichlet(rep_vector(1, nthres+1),0", class = "Intercept"),
                     set_prior("normal(0,1)", class = "sd"),
                     set_prior("normal(0,1)", class = "b")),
...
```{r, include = FALSE}
mn_ALL <-  brm(fruit ~  log_cuberoot_mass*m1_opcr + 
...
ms_ALL <-  brm(fruit ~  log_cuberoot_mass*m1_opcr + 
                      log_cuberoot_mass*m2_opcr +
                      log_cuberoot_mass*m1_dne  + 
                      log_cuberoot_mass*m2_dne  + 
                      log_cuberoot_mass*RFI_m1  + 
...
md_ALL <-  brm(fruit~  log_cuberoot_mass*m1_opcr + 
                      log_cuberoot_mass*m2_opcr +
                      log_cuberoot_mass*m1_dne  + 
                      log_cuberoot_mass*m2_dne  + 
                      log_cuberoot_mass*RFI_m1  + 
...
mw_fruit <- model_weights(mn_DNE, ms_DNE, md_DNE, mn_ALL, ms_ALL, md_ALL, mn_OPCR, ms_OPCR, md_OPCR, mn_RFI, ms_RFI, md_RFI, mn_RLGA,ms_RLGA, md_RLGA, weights = "stacking") %>% 
  round(digits = 3) %>% 
  as.data.frame() %>%
  rownames_to_column("metric") %>% 
  add_column(diet = "fruit") %>% 
  rename("value" = ".") %>% 
  arrange(desc(value))
...
#### Write Weights
```{r}
mw_fruit <- model_weights( md_DNE, ms_ALL, ms_OPCR, mn_RFI, ms_RLGA, weights = "stacking") %>% 
  round(digits = 3) %>% 
  as.data.frame() %>%
  rownames_to_column("metric") %>% 
  add_column(diet = "fruit") %>% 
...
```{r}
ms_OPCR %>% write_rds("new_mod_outputs/fruit_ms_OPCR.Rds")
ms_RLGA %>% write_rds("new_mod_outputs/fruit_ms_RLGA.Rds")
mn_RFI %>%  write_rds("new_mod_outputs/fruit_mn_RFI.Rds")
```

...
```{r}
set.seed(826)
mw_egg <- model_weights( mn_DNE, mn_ALL, md_OPCR, mn_RFI, md_RLGA, weights = "stacking") %>% 
  round(digits = 8) %>% 
  as.data.frame() %>%
  rownames_to_column("metric") %>% 
  add_column(diet = "egg") %>% 
...
```{r}
mn_DNE %>% write_rds(file = here("Code/Models/mod_outputs", "egg_mn_DNE.Rds"))
mn_RFI %>% write_rds(file = here("Code/Models/mod_outputs", "egg_mn_RFI.Rds"))
md_RLGA %>% write_rds(file = here("Code/Models/mod_outputs", "egg_md_RLGA.Rds"))
mn_ALL %>% write_rds(file = here("Code/Models/mod_outputs", "egg_mn_ALL.Rds"))
md_OPCR %>% write_rds(file = here("Code/Models/mod_outputs", "egg_md_OPCR.Rds"))
```

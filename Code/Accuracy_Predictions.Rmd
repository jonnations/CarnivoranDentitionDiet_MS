...
pred_means <- function(dfs){
  dfs %>%  mutate(pred = as.numeric(as.character(pred)),
               ...
               w_p_waic = p_waic*weight) %>% 
  group_by(Species, item) %>% 
  summarise(actual = mean(actual),
            sd_pred = sd(pred),
            pred = mean(pred),
            pareto_k = mean(w_pareto),
            p_waic = mean(w_p_waic)) %>% 
  mutate_at(c('pareto_k','p_waic'),  round, digits = 3) %>% 
...
pred_db %>% 
  ...
  select(!c(n2, n1)) %>% 
  arrange(desc(perfect_match)) %>% write_csv(file = here("Data", "Accuracy_Table_Cont.csv")) #%>% 

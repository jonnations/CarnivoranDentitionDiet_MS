...
```{r}
pred.root_RLGA <- 
  epred_draws(readRDS(here("Code/Models/mod_outputs", "root_mn_RLGA.Rds")), newdata = dat, ndraws = NULL) %>% 
    as.data.frame() %>% 
    select(Species, .draw, .category, .epred) %>% 
    arrange(Species, .draw, .category) %>% 
    pivot_wider(names_from = .category, values_from = .epred) %>% 
...
```{r}
pred.root_RLGA <- 
  epred_draws(readRDS(here("Code/Models/mod_outputs", "root_mn_RLGA.Rds")), newdata = cfdat, allow_new_levels = T, ndraws = NULL) %>% 
    as.data.frame() %>% 
    select(Species, .draw, .category, .epred) %>% 
    arrange(Species, .draw, .category) %>% 
    pivot_wider(names_from = .category, values_from = .epred) %>% 
